...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2313923 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
...
void proc_run(struct proc_struct *proc)
{
    ...
    //LAB8 2313923 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2313923 HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
...
{
    /* LAB8:EXERCISE2 2313923  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    {
        if ((ret = load_icode_read(fd, &ph, sizeof(ph), elf.e_phoff + sizeof(ph) * i)) != 0)
        {
        ...
        for (uintptr_t la = ROUNDDOWN(start, PGSIZE); la < end; la += PGSIZE)
        {
            ...
            uintptr_t page_start = la, page_end = la + PGSIZE;
            uintptr_t copy_start = (page_start < start) ? start : page_start;
            uintptr_t copy_end = (page_end < file_end) ? page_end : file_end;

            if (copy_end > copy_start)
            {
                size_t copy_len = copy_end - copy_start;
                off_t off = ph.p_offset + (copy_start - start);
                if ((ret = load_icode_read(fd, kva + (copy_start - la), copy_len, off)) != 0)
                {
                    goto bad_cleanup_mmap;
                }
            }
    ...
    if ((page = pgdir_alloc_page(mm->pgdir, USTACKTOP - PGSIZE, PTE_U | PTE_R | PTE_W)) == NULL)
    {
        ret = -E_NO_MEM;
        goto bad_cleanup_mmap;
    }
    ...
    uintptr_t stack_top = USTACKTOP, stack_base = USTACKTOP - PGSIZE;
    char *stk_kva = (char *)page2kva(page);
    for (int i = argc - 1; i >= 0; i--)
    {
        ...
        memcpy(stk_kva + (stack_top - stack_base), kargv[i], len);
        uargv[i] = stack_top;
    }

    stack_top = ROUNDDOWN(stack_top, sizeof(uintptr_t));
    stack_top -= sizeof(uintptr_t) * (argc + 1);
    if (stack_top < stack_base)
    {
        ret = -E_NO_MEM;
        goto bad_cleanup_mmap;
    ...
    uintptr_t *argv_store = (uintptr_t *)(stk_kva + (stack_top - stack_base));
    for (int i = 0; i < argc; i++)
    {
        argv_store[i] = uargv[i];
    }
    argv_store[argc] = 0;
